...
def gen_alpha_values(
    ...
    if num_minority > 0:
        if minority_dist == "unique":
            minority_values = np.arange(1, num_minority + 1, dtype=np.uint32)
        elif minority_dist == "zipfian":
            minority_values = rng.zipf(1.5, size=num_minority).astype(np.uint32)
        elif minority_dist == "uniform_100":
            minority_values = rng.integers(1, 101, size=num_minority, dtype=np.uint32)
        elif minority_dist == "two_value":
            minority_values = np.ones(num_minority, dtype=np.uint32)
        else:
