...
def find_optimal_bloom(keys, values, num_minority, bpe_range=[1, 2, 3, 4], nh_range=[1, 2, 3, 4]):
...
def plot_step_function(results, output_path: str, show: bool = False):
    ...
    if filter_type == 'bloom':
        ...
        ax1.step(alphas, results['optimal_bpe'], where='mid', linewidth=2.5, color='#1f77b4')
        ...
        # Plot num_hashes
        ax2.step(alphas, results['optimal_nh'], where='mid', linewidth=2.5, color='#2ca02c')
        ...
        ax.step(alphas, results['optimal_bits'], where='mid', linewidth=2.5, color=color)
        ...
        ax.set_title(f'{filter_type.title()}: Optimal Fingerprint Bits vs α (N={n:,})', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(description='Analyze step functions for optimal filter parameters')
    parser.add_argument('--filter', '-f', type=str, default='binaryfuse',
    ...
    parser.add_argument('--alpha-end', type=float, default=0.96,
