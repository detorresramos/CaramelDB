...
/*
  ...
  // Requires c-style (row-major) arrays due to assumptions in pointer indexing.
  // Typedefs: RowsPtr is a pointer to a list of rows indices in the matrix.
  // ColumnMap is a map from column indices to a row list.
  // PriorityQueue ranks (value, rows containing value) entries by list length.
  using RowsPtr = std::shared_ptr<std::vector<int>>;
  struct CompareLength  {  // Does not check for null dereference.
    constexpr bool operator() (RowsPtr const& a, RowsPtr const& b)
        const noexcept { return a.second->size() < b.second->size(); }
  };
  using SortedMap = std::map<V, RowsPtr, CompareLength>;

  using PriorityQueue = std::priority_queue
      <VectorPtr, std::vector<VectorPtr>, CompareLength>;
...
//     constexpr bool operator()(SwapOption const &a, SwapOption const &b)
//         const noexcept { return a.entropy < b.entropy; }
// };

// typedef std::priority_queue<SwapOption, std::vector<SwapOption>, CompareOptions> PriorityQueue;

// entropyPermutationSwaps<T>(py::array_t<T> &array){
//     // 1. Get the vocabulary.

//     // 2. Construct a priority queue.
//     // Invariant: the top swap should be the biggest entropy reduction.
//     PriorityQueue queue;
