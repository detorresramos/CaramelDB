...
void entropyPermutation(T *M, int num_rows, int num_cols) {
  ...
  auto max_location =
      std::max_element(val_to_row.begin(), val_to_row.end(),
                       [](const std::pair<T, RowList> &a,
                          const std::pair<T, RowList> &b) -> bool {
  ...
  while (num_to_assign > 0) {
    ...
    for (const int &row : best_rows) {
      ...
      for (int col = 0; col < num_cols; col++) {
        ...
        while (running_locations[col] < (eligible_rows[col].size() - 1)) {
          ...
          if (eligible_rows[col][running_locations[col]] >= row) {
            break;
          }
          running_locations[col]++;
        }
        running_locations[col] =
            std::min(running_locations[col], eligible_rows[col].size());
      }
    ...
    std::set_difference(eligible_rows[best_col].begin(),
                        eligible_rows[best_col].end(), best_rows.begin(),
                        best_rows.end(),
    ...
    std::set_difference(val_to_row[best_value].begin(),
                        val_to_row[best_value].end(), all_assigned.begin(),
                        all_assigned.end(),
    ...
    val_to_row[best_value] =
        rows_with_unassigned_best_value; // TODO: std::move.
  }
}
